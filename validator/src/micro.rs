    ...
    async fn next(
        ...
        let result = {
            ...
            if !in_current_state(head) {
                ...
                // Use a trusted push since these blocks were generated by this validator
                let result = if cfg!(feature = "trusted_push") {
                    Blockchain::trusted_push(blockchain, Block::Micro(skip_block.clone()), &block_publisher)
                } else {
                    Blockchain::push(blockchain, Block::Micro(skip_block.clone()), &block_publisher)
                };
